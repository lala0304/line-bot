...
def schedule_task():
    ...
    # 設置不同時段的定時任務
    schedule.every().day.at("07:00").do(send_drink_water_reminder, message='🌞 早安！新的開始，先來一杯清新的水，喚醒一整天的活力吧！')
    schedule.every().day.at("09:00").do(send_drink_water_reminder, message='🚀 工作要有衝勁，別忘了喝水提神哦！💧')
    schedule.every().day.at("11:30").do(send_drink_water_reminder, message='🍽 午餐時間快到了，先喝杯水，準備迎接美味吧！')
    schedule.every().day.at("13:00").do(send_drink_water_reminder, message='☕ 午餐後的一杯水，有助於消化，保持健康！')
    schedule.every().day.at("15:30").do(send_drink_water_reminder, message='🌟 下午茶時間到了，來杯水，保持頭腦清醒，繼續高效工作！')
    schedule.every().day.at("17:30").do(send_drink_water_reminder, message='🌅 工作接近尾聲，來杯水，給今天畫個完美的句號！')
    schedule.every().day.at("19:00").do(send_drink_water_reminder, message='🌙 晚飯時間到了，先來一杯水，幫助消化更健康！')
    schedule.every().day.at("21:30").do(send_drink_water_reminder, message='🌜 睡前喝杯水，保持身體水分充足，迎接美好的夢境！')
